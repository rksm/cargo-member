...
fn mv() -> anyhow::Result<()> {
    ...
    fs::write(tempdir_path.join("ws").join("Cargo.toml"), ORIGINAL)?;
    ...
    cargo_member::Mv::from_metadata(&metadata, "b", &tempdir_path.join("b"))
        .dry_run(false)
        .stderr(NoColor::new(&mut stderr))
        .exec()?;

    assert_manifest(
        &tempdir_path.join("ws").join("Cargo.toml"),
        EXPECTED_MANIFEST,
    )?;
